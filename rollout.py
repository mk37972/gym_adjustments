    ...
    def generate_rollouts(self):
        ...
        for t in range(self.T):
            ...
            if self.compute_Q:
                u, Q = policy_output
                Qs.append(Q)
                Fs.append(np.abs(np.float32(o[:,11:13].sum(axis=-1))).mean())
            else:
                u = policy_output
            if u.ndim == 1:
...
#            self.venv.render()
            ...
            success2 = (np.float32(o[:,11:13].sum(axis=-1))*1000 > -147.150/3*6)
...
#        print("--------------------New Rollout--------------------")
        ...
        success_rate = np.mean(successful*successful2.mean(axis=0))
        self.success_history.append(success_rate)
        if self.compute_Q:
            self.Q_history.append(np.mean(Fs))
