...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.fragile_on:
            ...
            if self.reward_type == 'sparse':
                return -(d > self.distance_threshold).astype(np.float32) - (np.float32(fragile_goal.sum(axis=-1)))/200.0
            else:
                return -d
        else:
    ...
    def _set_action(self, action):
        ...
        stiffness_ctrl *= 200.0 if action.shape == (5,) else 0.0
...
#                self.broken_object = 2.0
            ...
            conc_stiffness_data = np.concatenate([[l_finger_force,
                                 r_finger_force,
                                 self.prev_stiffness,
                                 self.psv_prev_stiffness]])/4000.0
