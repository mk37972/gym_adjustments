...
MANIPULATE_WALL_XML = os.path.join('hand', 'manipulate_egg_touch_sensors_wall.xml')
...
class ManipulateTouchSensorsEnv(manipulate.ManipulateEnv):
    def __init__(
        ...
        manipulate.ManipulateEnv.__init__(
            ...
            ignore_z_target_rotation=ignore_z_target_rotation, nAction=nAction
        )
    ...
    def _get_obs(self):
        ...
        achieved_goal = np.concatenate([self._get_achieved_goal().ravel(), fingertip_pos])  # this contains the object position + rotation
        ...
        observation = np.concatenate([robot_qpos, fingertip_pos, touch_values, achieved_goal])
...
class HandWallTouchSensorsEnv(ManipulateTouchSensorsEnv, utils.EzPickle):
    def __init__(self, target_position='random', target_rotation='ignore', touch_get_obs='sensordata', reward_type='sparse'):
        utils.EzPickle.__init__(self, target_position, target_rotation, touch_get_obs, reward_type)
        ManipulateTouchSensorsEnv.__init__(self,
            model_path=MANIPULATE_WALL_XML,
            touch_get_obs=touch_get_obs,
            target_rotation=target_rotation,
            target_position=target_position,
            target_position_range=np.array([(-0.04, 0.04), (-0.06, 0.02), (0.0, 0.06)]),
