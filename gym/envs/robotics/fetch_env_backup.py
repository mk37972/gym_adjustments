    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.fragile_on:
            try: 
                ...
                fragile_goal = np.linalg.norm(achieved_goal[:,3:5] - goal[:,3:5], axis=-1)
                penalty = np.linalg.norm(achieved_goal[:,5] - goal[:,5], axis=-1)
            except: 
                d = goal_distance(achieved_goal[:3], goal[:3])
                fragile_goal = np.linalg.norm(achieved_goal[3:5] - goal[3:5])
                penalty = np.linalg.norm(achieved_goal[5] - goal[5])
            if self.reward_type == 'sparse':
                return -(d > self.distance_threshold).astype(np.float32) - (np.float32(fragile_goal.sum(axis=-1)))/20.0# - penalty * 1.0 
            else:
                return -d
        else:
...
#                self.broken_table = True
            ...
            l_finger_force = np.min([l_finger_force, 0.0])
            r_finger_force = np.min([r_finger_force, 0.0])
            ...
            if (object_force > self.object_fragility):
                ...
                self.broken_object = 250.0
#                self.sim.model.body_mass[self.sim.model.body_name2id('object0')] = 100.0
...
#                self.broken_object = 2.0
            if self.n_actions == 4:
                conc_stiffness_data = np.concatenate([[l_finger_force,
                                     r_finger_force,
                                     self.broken_object]])/self.force_scale
            elif self.n_actions == 5:
                conc_stiffness_data = np.concatenate([[l_finger_force,
                                     r_finger_force,
                                     self.broken_object,
                                     self.prev_stiffness]])/self.force_scale
            elif self.n_actions == 6:
                conc_stiffness_data = np.concatenate([[l_finger_force,
                                     r_finger_force,
                                     self.broken_object,
                                     self.prev_stiffness,
                                     self.par_prev_stiffness + self.psv_prev_stiffness]])/self.force_scale
                ...
                conc_stiffness_data = np.concatenate([[l_finger_force,
                                     ...
                                     self.par_prev_stiffness]])/self.force_scale
            
    
            obs = np.concatenate([
                grip_rel_pos, object_rel_pos.ravel(), gripper_state, goal_rel_pos.ravel(), conc_stiffness_data
            ])
    
            achieved_goal = np.concatenate([achieved_goal, conc_stiffness_data[:3]])
...
#            ])
    ...
    def _sample_goal(self):
        ...
        if self.fragile_on: return np.concatenate([goal.copy(), [0.0, 0.0, 0.0]])
    ...
    def _is_success(self, achieved_goal, desired_goal):
        if self.fragile_on:
            try: 
                ...
                fragile_goal = np.linalg.norm(achieved_goal[:,3:5] - desired_goal[:,3:5], axis=-1)
            except: 
                d = goal_distance(achieved_goal[:3], desired_goal[:3])
                fragile_goal = np.linalg.norm(achieved_goal[3:5] - desired_goal[3:5])
        else: d = goal_distance(achieved_goal, desired_goal)
#        return (((d < self.distance_threshold).astype(np.float32) + np.float32(fragile_goal.sum(axis=-1)*100.0 < self.object_fragility)) == 2.0).astype(np.float32) if self.fragile_on else (d < self.distance_threshold).astype(np.float32)
        return (d < self.distance_threshold).astype(np.float32)
