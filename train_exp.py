...
# for dim in [5]:
#     for seed in [1000]:
#         savepath = '--save_path=./models/chip/vel/fpp_demo25bad{}dim_{}'.format(dim,seed)
#         demofile = '--demo_file=./gym_adjustments/data_chip_vel_random_25_bad{}dim.npz'.format(dim)
#         logpath = '--log_path=./models/chip/vel/fpp_demo25bad{}dim_{}_log'.format(dim,seed)
#         perturb = '--perturb=none'
#         algdim = '--algdim={}'.format(dim)
# #        if seed >= 100 and seed < 1000: seed = 10
# #        elif seed >= 1000 and seed < 10000: seed = 100
...
for dim in [6]:
    for seed in [10,100,1000]:
        savepath = '--save_path=./models/noreplacement/fpp_demo25bad{}dim_{}'.format(dim,seed)
        demofile = '--demo_file=./gym_adjustments/data_fetch_random_25_bad{}dim.npz'.format(dim)
        logpath = '--log_path=./models/noreplacement/fpp_demo25bad{}dim_{}_log'.format(dim,seed)
        perturb = '--perturb=none'
        algdim = '--algdim={}'.format(dim)
#        if seed >= 100 and seed < 1000: seed = 10
#        elif seed >= 1000 and seed < 10000: seed = 100
