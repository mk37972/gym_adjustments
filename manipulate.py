...
class ManipulateEnv(hand_env.HandEnv):
    def __init__(
        ...
        ignore_z_target_rotation=False, nAction=20
    ):
        ...
        hand_env.HandEnv.__init__(
            ...
            relative_control=relative_control, nAction=nAction)
...
#        achieved_both = achieved_pos * achieved_rot
        return achieved_both if self.nAction == 20 else (np.linalg.norm(achieved_goal[2]-achieved_goal[8]) < 0.02).astype(np.float32)
    ...
    def _reset_sim(self):
        ...
        if self.randomize_initial_rotation and self.nAction == 20:
        ...
        if self.randomize_initial_position and self.nAction == 20:
            if self.target_position != 'fixed':
                initial_pos += self.np_random.normal(size=3, scale=0.005)
        elif self.nAction == 3:
            initial_pos = self.sim.data.get_site_xpos('robot0:T_fftip_tip').copy() + np.array([0.02,0.0015,0.0025])
...
#        for _ in range(10):
#            self._set_action(np.zeros(self.nAction)) if self.nAction == 20 else self._set_action(np.array([0,0,0,1]))
#            try:
#                self.sim.step()
